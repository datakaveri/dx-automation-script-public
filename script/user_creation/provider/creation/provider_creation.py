#!/usr/bin/env python3
"""Create one provider user on the platform.

Usage:
    python provider_creation.py provider_creation_config.json
    python provider_creation.py provider_creation_config.json --mode join_and_grant
    python provider_creation.py provider_creation_config.json --dry-run

The platform never hands out the `provider` role directly: a user earns it by
belonging to an organisation and being elevated by that organisation's admin, or
by having an organisation of their own approved. Which of those this script
drives is `provider.mode`:

    join_and_request  join a configured organisation, the org admin approves the
                      join, the user requests the provider role, the org admin
                      approves that too. The full platform path, and the one
                      that yields a plain provider. (default)
    join_and_grant    join a configured organisation, the org admin approves the
                      join, then the org admin grants the provider role outright
                      with POST /organization/user/provider — no request to
                      approve.
    org_create        the user asks for an organisation of their own and the COS
                      admin approves it. That approval grants `provider` *and*
                      `org_admin` together, so the account is an organisation
                      admin as well — and the deletion script then has to fall
                      back to Keycloak, since the platform refuses to
                      self-delete org admins.
    keycloak_role     assign the `provider` realm role straight through the
                      Keycloak Admin API. Fast, but it creates no organisation
                      membership and no database rows, so the account is not a
                      working provider — useful only for token and role checks.

The join modes need an existing organisation and its admin's credentials;
org_create needs a COS admin's. Both go in the config, along with every URL,
endpoint path, payload field and step toggle the script uses. Values may
reference the environment as ${VAR} or ${VAR:-fallback}.

What the account became is written to the handoff file named by `output.file`,
which the deletion script beside this one reads.
"""
...
LOG = logging.getLogger("provider_creation")

MODES = ("join_and_request", "join_and_grant", "org_create", "keycloak_role")
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    # The organisation the user joins, in the join modes. Give the id, or a name
    # to look up in the organisation list.
    "organisation": {
        "id": "",
        "name": "",
        "lookup_by_name": True,
    },
    # Whoever approves. A ready-made token is used as-is; otherwise the script
    # signs the account in with the password grant.
    "org_admin": {
        "username": "",
        "password": "",
        "token": "",
    ...
    "provider": {
        "mode": "join_and_request",
        "role_name": "provider",
        "refresh_token_after_join": True,
        "join": {
            "job_title": "Engineer",
            "emp_id": "EMP001",
            # Blank uses the new account's own email. It must be unique across
            # every organisation member on the platform.
            "official_email": "",
            "approve": True,
        ...
        "role_request": {
            # self: read the user's own provider requests to find the id.
            # org_admin: read the org admin's pending list instead.
            "lookup": "self",
            "approve": True,
            "approve_status": "granted",
            "settle_seconds": 2,
    ...
    "create": {
        # Sign in as the new account with the password grant. Every mode but
        # keycloak_role acts as the user, so they need this; turn it off only
        # to stop before a sign-in that is expected to fail.
        "fetch_token": True,
        "touch_control_plane": True,
        "expect_roles": ["provider"],
    ...
    "endpoints": {
        ...
        "cp_organisations": "/iudx/v2/auth/organisations",
        "cp_join_requests": "/iudx/v2/auth/organisations/{org_id}/join_requests",
        "cp_join_request": "/iudx/v2/auth/organisations/{org_id}/join_requests/{req_id}",
        "cp_provider_role_requests": "/iudx/v2/auth/organization/user/provider_role/requests",
        "cp_provider_role_request":
            "/iudx/v2/auth/organization/user/provider_role/requests/{req_id}",
        "cp_own_provider_requests": "/iudx/v2/auth/organization/user/provider_requests",
        "cp_provider_grant": "/iudx/v2/auth/organization/user/provider",
    ...
    "output": {
        ...
        "file": "provider_created.json",
        # The deletion script signs in as the account to use ControlPlane's
        # self-delete endpoint, so it needs the password from somewhere.
...
def resolve_org_id(config: dict[str, Any], cp: ControlPlane, token: str) -> str:
    """The organisation to join: the configured id, or the one with that name."""
    org = config["organisation"]
    org_id = str(org.get("id") or "").strip()
    if org_id:
        return org_id
    name = str(org.get("name") or "").strip()
    if not name or not org.get("lookup_by_name", True):
        raise ConfigError(
            "set 'organisation.id', or 'organisation.name' with "
            "'organisation.lookup_by_name' enabled"
        )
    payload = cp.request(
        "GET", cp_path(config, "cp_organisations"), "list organisations", token=token
    )
    for row in rows_of(payload):
        if str(field(row, "name", "orgName") or "").strip() == name:
            found = str(field(row, "id", "organizationId", "organisationId") or "")
            if found:
                LOG.info("Organisation %s is %s", name, found)
                return found
    raise ApiError(f"no organisation named {name} is visible to this account")


def join_organisation(
    config: dict[str, Any], cp: ControlPlane, user_token: str, admin_token: str,
    org_id: str, user_id: str, email: str,
) -> str | None:
    """Submit a join request and have the org admin approve it."""
    join = config["provider"]["join"]
    official_email = str(join.get("official_email") or "").strip() or email
    cp.request(
        "POST", cp_path(config, "cp_join_requests", org_id=org_id),
        "submit join request", token=user_token,
        json_body={
            "job_title": str(join.get("job_title") or "Engineer"),
            "emp_id": str(join.get("emp_id") or "EMP001"),
            "official_email": official_email,
        },
    )
    LOG.info("Submitted a join request for organisation %s", org_id)

    if not join.get("approve", True):
        LOG.info("provider.join.approve is false — leaving the request pending")
        return None

    request_id = find_join_request(config, cp, admin_token, org_id, user_id, official_email)
    LOG.info("Org admin sees join request %s", request_id)
    cp.request(
        "PUT", cp_path(config, "cp_join_request", org_id=org_id, req_id=request_id),
        "approve join request", token=admin_token,
        json_body={"status": str(join.get("approve_status") or "granted")},
    )
    LOG.info("Join request approved — the user is a member of %s", org_id)

    settle = float(join.get("settle_seconds") or 0)
    if settle > 0:
        time.sleep(settle)
    return request_id
...
def find_join_request(
    config: dict[str, Any], cp: ControlPlane, admin_token: str, org_id: str,
    user_id: str, official_email: str,
) -> str:
    """Locate the user's pending join request in the org admin's list."""
    join = config["provider"]["join"]
    size = int(join.get("list_page_size") or 100)
    max_pages = int(join.get("list_max_pages") or 20)
    for page in range(1, max_pages + 1):
        payload = cp.request(
            "GET", cp_path(config, "cp_join_requests", org_id=org_id),
            "list join requests", token=admin_token,
            params={
                "status": str(join.get("list_status") or "pending"),
                "page": page,
                "size": size,
            },
        )
        ...
        for row in rows:
            same_user = str(field(row, "userId", "user_id") or "") == user_id
            same_email = str(field(row, "officialEmail", "official_email") or "") == official_email
        ...
        if len(rows) < size:
            break
    raise ApiError(f"no pending join request for {user_id} is visible to the org admin")


def request_provider_role(
    config: dict[str, Any], cp: ControlPlane, user_token: str, admin_token: str, user_id: str
) -> str | None:
    """Ask for the provider role, then have the org admin approve it."""
    role_request = config["provider"]["role_request"]
    cp.request(
        "POST", cp_path(config, "cp_provider_role_requests"),
        "request provider role", token=user_token, json_body={},
    )
    LOG.info("Requested the provider role")

    if not role_request.get("approve", True):
        LOG.info("provider.role_request.approve is false — leaving the request pending")
        return None

    request_id = find_provider_request(config, cp, user_token, admin_token, user_id)
    LOG.info("Provider role request is %s", request_id)
    cp.request(
        "PUT", cp_path(config, "cp_provider_role_request", req_id=request_id),
        "approve provider role request", token=admin_token,
        json_body={"status": str(role_request.get("approve_status") or "granted")},
    )
    LOG.info("Provider role request approved")

    settle = float(role_request.get("settle_seconds") or 0)
    if settle > 0:
        time.sleep(settle)
    return request_id
...
def find_provider_request(
    ...
    """The id of the pending provider role request, from either listing."""
    lookup = str(config["provider"]["role_request"].get("lookup") or "self").lower()
    if lookup == "self":
        payload = cp.request(
            "GET", cp_path(config, "cp_own_provider_requests"),
            "read own provider requests", token=user_token,
        )
        for row in rows_of(payload):
            if str(field(row, "status") or "").lower() == "pending":
                ...
                if request_id:
                    return request_id
        raise ApiError("the user has no pending provider role request")

    payload = cp.request(
        "GET", cp_path(config, "cp_provider_role_requests"),
        "list provider role requests", token=admin_token,
    )
    for row in rows_of(payload):
        if str(field(row, "user_id", "userId") or "") == user_id:
            ...
            if request_id:
                return request_id
    raise ApiError(f"no provider role request for {user_id} is visible to the org admin")


def grant_provider_role(
    config: dict[str, Any], cp: ControlPlane, admin_token: str, org_id: str, user_id: str
) -> None:
    """Org admin grants the provider role outright, with no request to approve."""
    cp.request(
        "POST", cp_path(config, "cp_provider_grant"),
        "grant provider role", token=admin_token,
        json_body={"user_id": user_id, "organization_id": org_id, "status": "granted"},
    )
    LOG.info("Org admin granted the provider role directly")


def create_own_organisation(
    config: dict[str, Any], kc: Keycloak, cp: ControlPlane, user_token: str, email: str
) -> dict[str, Any]:
    """Submit an organisation create request and have the COS admin approve it.

    The approval grants `provider` and `org_admin` together, so the account this
    produces is an organisation administrator as well as a provider.
    """
    org_create = config["provider"]["org_create"]
    name = str(org_create.get("name") or "").strip()
    if not name:
        stamp = datetime.now(timezone.utc).strftime("%m%d%H%M%S")
        name = f"{org_create.get('name_prefix') or 'prov-org'}-{stamp}"
    limit = int(org_create.get("name_max_length") or 0)
    if limit > 0:
        name = name[:limit]

    payload = dict(org_create.get("payload") or {})
    payload["name"] = name
    ...
    result: dict[str, Any] = {"org_name": name, "org_request_id": None}
    if not org_create.get("approve", True):
        LOG.info("provider.org_create.approve is false — leaving the request pending")
        return result

    cos_token = actor_token(kc, config["cos_admin"], "cos_admin")
    request_id = find_org_create_request(config, cp, cos_token, name)
    LOG.info("COS admin sees org create request %s", request_id)
    cp.request(
        "POST", cp_path(config, "cp_org_create_approve"),
        "approve org create request", token=cos_token,
        json_body={
            "req_id": request_id,
            "status": str(org_create.get("approve_status") or "granted"),
        },
    )
    LOG.info("Organisation create request approved")
    result["org_request_id"] = request_id
    return result
...
def find_org_create_request(
    ...
    """Locate the pending organisation create request by its name."""
    org_create = config["provider"]["org_create"]
    size = int(org_create.get("list_page_size") or 100)
    max_pages = int(org_create.get("list_max_pages") or 20)
    for page in range(1, max_pages + 1):
        payload = cp.request(
...
def create_provider(
    config: dict[str, Any], kc: Keycloak, cp: ControlPlane, username: str, email: str, mode: str
) -> dict[str, Any]:
    """Create the account, elevate it to provider, and return what teardown needs."""
    create = config["create"]
    ...
    user_token = ""
    if create.get("fetch_token", True):
        user_token = kc.user_token(username, password)
    ...
    if mode == "keycloak_role":
        role_name = str(provider.get("role_name") or "provider")
        kc.assign_realm_role(user_id, role_name)
        LOG.info("Assigned the %s realm role directly — no organisation, no database rows",
                 role_name)

    elif mode == "org_create":
        record.update(create_own_organisation(config, kc, cp, user_token, email))
        # The approval writes roles and the organisation attribute into Keycloak.
        user_token = kc.user_token(username, password)
    ...
    elif mode in ("join_and_request", "join_and_grant"):
        admin_token = actor_token(kc, config["org_admin"], "org_admin")
        org_id = resolve_org_id(config, cp, admin_token)
        record["org_id"] = org_id
        record["join_request_id"] = join_organisation(
            config, cp, user_token, admin_token, org_id, user_id, email
        )
        if provider.get("refresh_token_after_join", True):
            # Membership is written into Keycloak; the token predates that write.
            user_token = kc.user_token(username, password)
        if mode == "join_and_request":
            record["provider_request_id"] = request_provider_role(
                config, cp, user_token, admin_token, user_id
            )
        else:
            grant_provider_role(config, cp, admin_token, org_id, user_id)

    else:
        raise ConfigError(f"unknown provider.mode '{mode}'; expected one of {', '.join(MODES)}")
...
def run(config_path: Path, args: argparse.Namespace) -> int:
    ...
    mode = str(args.mode or config["provider"].get("mode") or "join_and_request").lower()
    if mode not in MODES:
        raise ConfigError(f"unknown provider.mode '{mode}'; expected one of {', '.join(MODES)}")

    if mode != "keycloak_role" and not config["create"].get("fetch_token", True):
        raise ConfigError(
            f"provider.mode '{mode}' acts as the user — it submits the join and "
            "role requests — so it needs the sign-in that create.fetch_token "
            "turns off. Set create.fetch_token to true, or use mode "
            "keycloak_role, which touches Keycloak only."
        )
    ...
    if args.dry_run:
        ...
        LOG.info("Would create provider %s (email %s)", username, email)
        if mode in ("join_and_request", "join_and_grant"):
            LOG.info(
                "Would join organisation %s and %s",
                config["organisation"].get("id") or config["organisation"].get("name") or "?",
                "request the provider role, approved by the org admin"
                if mode == "join_and_request"
                else "have the org admin grant the provider role",
            )
        elif mode == "org_create":
            LOG.info("Would create an organisation approved by the COS admin, "
                     "granting org_admin as well as provider")
        else:
            LOG.info("Would assign the %s realm role directly",
                     config["provider"].get("role_name"))
        if config["compute"].get("enabled", False):
            LOG.info(
    ...
    record = create_provider(config, kc, cp, username, email, mode)
    write_output(config, config_path, record)
    log_account(config, record)
    LOG.info("Created provider %s (%s)", record["username"], record["user_id"])
    return 0
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        default=Path(__file__).with_name("provider_creation_config.json"),
        help="JSON config path (default: provider_creation_config.json beside this script)",
    )
    parser.add_argument("--mode", choices=MODES, help="override provider.mode from the config")
