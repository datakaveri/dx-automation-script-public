#!/usr/bin/env python3
"""Delete one provider user from the platform.

Usage:
    python provider_deletion.py provider_deletion_config.json
    python provider_deletion.py provider_deletion_config.json --username someone@example.invalid
    python provider_deletion.py provider_deletion_config.json --dry-run
...
what lets a teardown run with no ids typed out by hand, and it also carries the
organisation the account belongs to.

`delete.mode` decides the route:

    auto      ControlPlane's own self-delete endpoint — it removes the provider
              role, the organisation membership, the join requests and the
              compute requests — and then the Keycloak Admin API for the
              account itself, which that endpoint leaves behind despite what it
              reports. Also falls back to the Admin API if the platform refuses
              outright. (default)
    api       self-delete only; fail if the platform refuses. The Keycloak
              account survives, so the run's verify step will report it.
    keycloak  the Keycloak Admin API only, leaving platform rows behind.

A provider made through one of the join modes self-deletes cleanly. One made
through `org_create` is an org admin too, and the platform refuses to
self-delete org admins — `auto` then falls back to Keycloak, and the
organisation is left behind; the org_admin deletion script beside this one is
what clears that.

`delete.remove_from_organisation` asks the org admin to remove the member first.
The self-delete route already unwinds membership, so it is off by default and is
there for the Keycloak route.
"""
...
LOG = logging.getLogger("provider_deletion")
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "target": {
        ...
        "password": "",
        # Only needed when removing the member from the organisation first.
        "org_id": "",
        # The record the creation script wrote, in its own folder.
        "input_file": "../creation/provider_created.json",
        "require_input_file": False,
        "remove_input_file": True,
    },
    # Needed only when delete.remove_from_organisation is on.
    "org_admin": {
        "username": "",
        "password": "",
        "token": "",
    ...
    "delete": {
        "mode": "auto",
        "remove_from_organisation": False,
        "pause_seconds": 0,
        "verify": True,
    ...
    "endpoints": {
        ...
        "cp_org_user": "/iudx/v2/auth/organisations/{org_id}/users/{user_id}",
    },
    "logging": {
        "level": "INFO",
...
def remove_from_organisation(
    config: dict[str, Any], kc: Keycloak, cp: ControlPlane, target: dict[str, Any]
) -> None:
    """Have the org admin drop the member before the account itself goes.

    A failure here is reported and not raised: the account still has to go, and
    the self-delete route unwinds membership anyway.
    """
    org_id = target.get("org_id") or ""
    user_id = target.get("user_id") or ""
    if not org_id or not user_id:
        LOG.warning(
            "Cannot remove the member: %s is unknown",
            "org_id" if not org_id else "user_id",
        )
        return
    try:
        admin_token = actor_token(kc, config["org_admin"], "org_admin")
        cp.request(
            "DELETE", cp_path(config, "cp_org_user", org_id=org_id, user_id=user_id),
            "remove user from organisation", token=admin_token,
        )
        LOG.info("Removed %s from organisation %s", target["username"], org_id)
    except (ApiError, ConfigError) as exc:
        LOG.warning("Could not remove the user from the organisation: %s", exc)
...
def delete_provider(
    ...
    if delete.get("remove_from_organisation", False):
        if not target.get("user_id"):
            found = kc.find_user(target["username"])
            if found:
                target["user_id"] = found["id"]
        remove_from_organisation(config, kc, cp, target)
    ...
    if mode in ("auto", "api"):
        try:
            ...
            if mode == "api":
                raise
            # Expected when the account is an org admin as well as a provider.
    ...
    if not removed:
        ...
        if target.get("org_id"):
            LOG.warning(
                "Organisation %s is untouched — the Keycloak route removes the account "
                "only. Use the org_admin deletion script to clear an organisation.",
                target["org_id"],
            )
...
def run(config_path: Path, args: argparse.Namespace) -> int:
    ...
    if args.dry_run:
        ...
        if config["delete"].get("remove_from_organisation", False):
            LOG.info("Would ask the org admin to remove the member from %s",
    ...
    delete_provider(config, kc, cp, target)
    consume_input_file(config, config_path, target["username"])
    LOG.info("Deleted provider %s", target["username"])
    return 0
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        default=Path(__file__).with_name("provider_deletion_config.json"),
        help="JSON config path (default: provider_deletion_config.json beside this script)",
    )
