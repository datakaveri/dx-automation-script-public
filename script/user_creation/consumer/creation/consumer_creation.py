#!/usr/bin/env python3
"""Create one consumer user on the platform.

Usage:
    python consumer_creation.py consumer_creation_config.json
    python consumer_creation.py consumer_creation_config.json --username someone@example.invalid
    python consumer_creation.py consumer_creation_config.json --dry-run

A consumer is a plain platform account: no organisation, no elevated role. It is
created through the Keycloak Admin API — ControlPlane has no create-user API, so
users originate in Keycloak — and then introduced to ControlPlane so the
platform-side record exists.
...
LOG = logging.getLogger("consumer_creation")
...
def create_consumer(
    ...
    """Create the account and return what the deletion script will need."""
    create = config["create"]
...
def run(config_path: Path, args: argparse.Namespace) -> int:
    ...
    LOG.info("Created consumer %s (%s)", record["username"], record["user_id"])
    return 0
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        default=Path(__file__).with_name("consumer_creation_config.json"),
        help="JSON config path (default: consumer_creation_config.json beside this script)",
    )
