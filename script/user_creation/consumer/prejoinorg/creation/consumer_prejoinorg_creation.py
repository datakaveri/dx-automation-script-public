#!/usr/bin/env python3
"""Create one consumer user that is a member of an existing organisation.

Usage:
    python consumer_prejoinorg_creation.py consumer_prejoinorg_creation_config.json
    python consumer_prejoinorg_creation.py consumer_prejoinorg_creation_config.json --username someone@example.invalid
    python consumer_prejoinorg_creation.py consumer_prejoinorg_creation_config.json --dry-run

A consumer is a plain platform account with no elevated role. This variant also
joins it to an organisation you already have: the account is created through
the Keycloak Admin API — ControlPlane has no create-user API, so users originate
in Keycloak — introduced to ControlPlane, and then submits a join request to the
organisation named in `organisation`, which that organisation's admin approves
with the credentials in `org_admin`. The account ends up a consumer *and* an
organisation member, with no provider role.

Name the organisation by id (`organisation.organisationId` / `organisation.id`)
or by name (`organisation.organisationName` / `organisation.name`); an id wins,
and a name alone is looked up in the organisation list. Set `join.approve` to
false to leave the join request pending instead — no org admin is needed then.

What the account was is written to the handoff file named by `output.file`,
which the deletion script beside this one reads, so a teardown needs no ids
typed out by hand.

Everything the script touches — URLs, credentials, endpoint paths, the user's own
fields, which roles to assert — comes from the JSON config. Values may reference
...
LOG = logging.getLogger("consumer_prejoinorg_creation")
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    # The organisation the consumer joins. Give the id, or a name to look up in
    # the organisation list. Both spellings of each key are read, so a block
    # copied from somewhere else can be pasted in as it is.
    "organisation": {
        "organisationId": "",
        "organisationName": "",
        "id": "",
    ...
    # That organisation's admin, who approves the join. Only read when
    # join.approve is true.
    "org_admin": {
        "username": "",
        "password": "",
        "token": "",
    ...
    "join": {
        # What the join request carries. official_email blank means the
        # account's own email.
        "job_title": "Engineer",
        "emp_id": "EMP001",
        "official_email": "",
        ...
        # How the org admin finds the request: which status to list and how
        # far to page.
        "list_status": "pending",
        "list_page_size": 100,
        "list_max_pages": 20,
        "settle_seconds": 2,
        # Membership is written into Keycloak; a token minted before the
        # approval does not carry it.
        "refresh_token_after_join": True,
        # After approval, wait for the organisation attribute to show up on
        # the Keycloak account and warn if it never does. Off skips the check.
        "verify_membership": True,
        "org_attribute": "organisation_id",
        "verify_timeout_seconds": 30,
        "verify_poll_seconds": 2,
    },
    # The compute role's approver. Only read when compute.enabled is true.
    "cos_admin": {
        "username": "",
        "password": "",
        "token": "",
    ...
    "output": {
        ...
        "file": "consumer_prejoinorg_created.json",
...
def configured_org(config: dict[str, Any]) -> tuple[str, str]:
    """The organisation id and name from the config, whichever spelling was used."""
    org = config["organisation"]
    org_id = str(org.get("organisationId") or org.get("id") or "").strip()
    name = str(org.get("organisationName") or org.get("name") or "").strip()
    return org_id, name


def resolve_org_id(config: dict[str, Any], cp: ControlPlane, token: str) -> tuple[str, str]:
    """The organisation to join: the configured id, or the one with that name.

    Returns the id and the name. With an id configured the name is whatever the
    config says, possibly blank; with a name alone the list is searched and the
    id comes from the row that matches.
    """
    org_id, name = configured_org(config)
    if org_id:
        return org_id, name
    if not name or not config["organisation"].get("lookup_by_name", True):
        raise ConfigError(
            "set 'organisation.organisationId' (or 'organisation.id'), or "
            "'organisation.organisationName' (or 'organisation.name') with "
    ...
    for row in rows_of(payload):
        if str(field(row, "name", "orgName", "organisationName") or "").strip() == name:
...
def join_organisation(
    ...
    if not join.get("approve", True):
        LOG.info("join.approve is false — leaving the request pending, so the "
                 "account is not a member yet")
        return None
...
def verify_membership(config: dict[str, Any], kc: Keycloak, user_id: str, org_id: str) -> bool:
    """Wait for the organisation attribute to land on the Keycloak account.

    The approval writes membership into Keycloak asynchronously, so this polls.
    A miss is reported, not raised: the approval call itself succeeded, and the
    attribute name is deployment-specific (`join.org_attribute`).
    """
    join = config["join"]
    if not join.get("verify_membership", True):
        return False
    name = str(join.get("org_attribute") or "organisation_id")
    deadline = time.monotonic() + float(join.get("verify_timeout_seconds") or 30)
    interval = float(join.get("verify_poll_seconds") or 2)
    while True:
        value = kc.attributes(user_id).get(name)
        values = value if isinstance(value, list) else ([value] if value else [])
        if any(str(item) == org_id for item in values):
            LOG.info("Keycloak shows %s=%s on the account", name, org_id)
            return True
        if time.monotonic() >= deadline:
            break
        time.sleep(interval)
    LOG.warning(
        "Keycloak attribute %s did not become %s within the timeout (has: %s) — "
        "the join was approved, but check the membership by hand",
        name, org_id, values or "nothing",
    )
    return False
...
def create_consumer(
    config: dict[str, Any], kc: Keycloak, cp: ControlPlane, username: str, email: str
) -> dict[str, Any]:
    """Create the account, join it to the organisation, and return what the
    deletion script will need."""
    create = config["create"]
    ...
    if create.get("fetch_token", True):
        token = kc.user_token(username, password)
    ...
    if create.get("touch_control_plane", True):
        if not token:
            token = kc.user_token(username, password)
        info = cp.request(
            "GET", cp_path(config, "cp_user_info"), "read own user info", token=token
        )
        if isinstance(info, dict):
            LOG.info(
                "ControlPlane knows the user: sub=%s roles=%s",
                field(info, "sub", "userId", "user_id") or "?",
                ", ".join(info.get("roles") or []) or "none",
            )

    if not token:
        # The join request is submitted as the account.
        token = kc.user_token(username, password)

    join = config["join"]
    admin_token = ""
    if join.get("approve", True):
        admin_token = actor_token(kc, config["org_admin"], "org_admin")
    # The name lookup runs as the org admin, who can certainly see their own
    # organisation; with approval off it runs as the user instead.
    org_id, org_name = resolve_org_id(config, cp, admin_token or token)
    join_request_id = join_organisation(config, cp, token, admin_token, org_id, user_id, email)
    member = join_request_id is not None
    membership_verified = False
    if member:
        if join.get("refresh_token_after_join", True):
            token = kc.user_token(username, password)
        membership_verified = verify_membership(config, kc, user_id, org_id)

    extra: dict[str, Any] = {}
    if config["compute"].get("enabled", False):
        extra = add_compute_role(config, kc, cp, token, username, password, user_id)

    roles = report_roles(config, kc, user_id)
...
def run(config_path: Path, args: argparse.Namespace) -> int:
    ...
    if args.dry_run:
        ...
        LOG.info("Would create consumer %s (email %s)", username, email)
        for role in config["user"].get("realm_roles") or []:
            LOG.info("Would assign realm role %s", role)
        org_id, org_name = configured_org(config)
        LOG.info(
            "Would join organisation %s and %s",
            org_id or org_name or "?",
            "have the org admin approve the join"
            if config["join"].get("approve", True)
            else "leave the join request pending",
        )
        if config["compute"].get("enabled", False):
            LOG.info(
    ...
    record = create_consumer(config, kc, cp, username, email)
    write_output(config, config_path, record)
    log_account(config, record)
    LOG.info(
        "Created consumer %s (%s) in organisation %s",
        record["username"], record["user_id"], record["org_id"],
    )
    return 0
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        default=Path(__file__).with_name("consumer_prejoinorg_creation_config.json"),
        help="JSON config path (default: consumer_prejoinorg_creation_config.json "
             "beside this script)",
    )
