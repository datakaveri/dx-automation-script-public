#!/usr/bin/env python3
"""Delete one consumer user that was joined to an organisation.

Usage:
    python consumer_prejoinorg_deletion.py consumer_prejoinorg_deletion_config.json
    python consumer_prejoinorg_deletion.py consumer_prejoinorg_deletion_config.json --username someone@example.invalid
    python consumer_prejoinorg_deletion.py consumer_prejoinorg_deletion_config.json --dry-run
...
`delete.mode` decides the route:
    ...
    auto      ControlPlane's own self-delete endpoint — it removes the
              organisation membership, the join requests and any compute
              requests — and then the Keycloak Admin API for the account
              itself, which that endpoint leaves behind despite what it
...
The platform refuses to self-delete org admins and platform admins; a consumer
that merely joined an organisation is neither, so `auto` normally takes the
cascading route. The organisation itself is never touched — it existed before
the account did.
...
there for the Keycloak route, which would otherwise leave the membership row
behind.
...
LOG = logging.getLogger("consumer_prejoinorg_deletion")
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "target": {
        ...
        "input_file": "../creation/consumer_prejoinorg_created.json",
...
def delete_consumer(
    ...
    if not removed:
        ...
        if target.get("org_id"):
            LOG.warning(
                "Membership of organisation %s may be left behind — the Keycloak route "
                "removes the account only. Turn on delete.remove_from_organisation "
                "to have the org admin drop the member first.",
                target["org_id"],
            )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        default=Path(__file__).with_name("consumer_prejoinorg_deletion_config.json"),
        help="JSON config path (default: consumer_prejoinorg_deletion_config.json "
             "beside this script)",
    )
