#!/usr/bin/env python3
"""Set the evaluation deadline of one challenge in dx-community-layer.

Usage:
    python challenge_evaluation_time.py challenge_evaluation_time_config.json
    python challenge_evaluation_time.py challenge_evaluation_time_config.json --competition-id <uuid>
    python challenge_evaluation_time.py challenge_evaluation_time_config.json --evaluation-end today
    python challenge_evaluation_time.py challenge_evaluation_time_config.json --publish
    python challenge_evaluation_time.py challenge_evaluation_time_config.json --dry-run

The deadline is `evaluation_ends_at`, written through
...
It comes from `times.evaluation_end` in the config, an expression: `today`, an
absolute `2026-10-01`, `today+21d`, or `submission_end+7d` (relative to the
submission window already on the server). "today" is accepted; the server only
insists on it falling after `submission_end` when publishing.
...
LOG = logging.getLogger("challenge_evaluation_time")
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    # null keeps the server's value. This script writes the evaluation
    # deadline; `creation` is only used when publishing (update.draft false):
    # a future time schedules the challenge for then, now or the past
    # publishes at once.
    "times": {
        "creation": "today",
...
MANAGED = (("evaluation_ends_at", "evaluation_end"),)
...
def apply_update(
    ...
    api.request(
        ...
        "update evaluation deadline", token, json_body=body, expect=expect,
    )
...
def run(config_path: Path, args: argparse.Namespace) -> int:
    ...
    if len(body) == 1 and not publishing:
        LOG.warning("times.evaluation_end is not set; nothing to change")
        return 0
    if publishing:
        LOG.warning(
    ...
    log_block("=== evaluation deadline ===", {
        "competition_id": competition_id,
        "title": detail.get("title") or title,
        "status": status,
        "submission_starts_at": current.get("submission_starts_at"),
        "submission_ends_at": current.get("submission_ends_at"),
        "evaluation_ends_at": f"{current.get('evaluation_ends_at')} → {body.get('evaluation_ends_at', '(unchanged)')}",
        "draft": body["draft"],
    ...
    LOG.info("Evaluation deadline set on challenge %s", competition_id)
    return 0
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        default=Path(__file__).with_name("challenge_evaluation_time_config.json"),
        help="JSON config path (default: challenge_evaluation_time_config.json beside this script)",
    )
